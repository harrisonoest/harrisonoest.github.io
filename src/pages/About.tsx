// === Mantine ===
import { Avatar, Badge, Box, Divider, Grid, List, Paper, Text, Title, useMantineTheme } from "@mantine/core";
...
export function About() {
  ...
  const skills = [
    { category: "Languages", items: ["TypeScript", "Rust", "C#", "Python", "Bash", "Go"] },
    { category: "Frameworks", items: ["React", "Node.js", "QML", "Qt"] },
    { category: "Databases", items: ["MongoDB", "PostgreSQL", "MySQL"] },
    { category: "Infrastructure", items: ["AWS", "NGINX", "Docker", "Linux", "GitLab CI/CD"] },
  ];
  ...
  return (
    <Box className={classes.container}>
      ...
      >
        <Grid gutter="xl">
          ...
          <Grid.Col span={{ base: 12, md: 8 }}>
            <Title order={1} className={classes.name}>Hello!</Title>
      ...
      >
        <Title order={2} mb="xl" className={classes.sectionTitle}>Skills</Title>

        <Grid>
          {skills.map((skillGroup) => (
            <Grid.Col key={skillGroup.category} span={{ base: 12, sm: 6, md: 3 }}>
              <Paper p="xl" radius="md" className={classes.skillCard}>
                <Title order={4} mb="sm">{skillGroup.category}</Title>
                <List spacing="xs">
      ...
      <ContentColumn
        ...
        backgroundColor={theme.colors.dark[7]}
      >
        <Title order={2} mb="xl" className={classes.sectionTitle}>Experience</Title>

        {experiences.map((exp, index) => (
          <Box key={exp.title} mb={index === experiences.length - 1 ? 0 : "xl"} w="100%">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
              <div>
                <Title order={4}>{exp.title}</Title>
                <Text size="sm" c="dimmed">{exp.company}</Text>
              </div>
              <Badge size="lg" style={{ textAlign: 'right', minWidth: '140px' }}>{exp.period}</Badge>
            </div>
      ...
      <ContentColumn
        ...
        backgroundColor={theme.colors.dark[8]}
      >
        <Title order={2} mb="xl" className={classes.sectionTitle}>Education</Title>

        {education.map((edu, index) => (
          <Box key={edu.degree} mb={index === education.length - 1 ? 0 : "xl"} w="100%">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
              <div>
                <Title order={4}>{edu.degree}</Title>
                <Text size="sm" c="dimmed">{edu.institution}</Text>
              </div>
              <Badge size="lg" style={{ textAlign: 'right', minWidth: '100px' }}>{edu.year}</Badge>
            </div>
      ...
      <ContentColumn
        ...
        backgroundColor={theme.colors.dark[7]}
      >
        <Title order={2} mb="xl" className={classes.sectionTitle}>Projects</Title>

        <Box mb="xl" w="100%">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
            <Title order={4}>PRimate</Title>
            <Text size="sm" style={{ textAlign: 'right' }}>
              <a href="https://github.com/harrisonoest/PRimate" target="_blank" rel="noopener noreferrer" className={classes.projectLink}>
          ...
          <List mt="md" spacing="xs">
            ...
            <List.Item>Reduces review time through automated notifications</List.Item>
            <List.Item>Enhances team collaboration with centralized feedback tracking</List.Item>
          </List>
        </Box>
